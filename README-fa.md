...
<p><strong>یک دانلود منیجر مدرن برای ویندوز مجهز به موتور چنداتصاله، دانلود ویدیو و صوت از شبکه‌های اجتماعی، دانلود گروهی، پشتیبانی از پروکسی، رابط کاربری 40 زبانه (شامل فارسی و انگلیسی)، گالری 27 تم و نسخه پرتابل و نصبی.</strong></p>
...
هر دو نسخه شامل تمامی ابزارهای موردنیاز برای دانلود ویدیو و آهنگ (`yt-dlp`، `ffmpeg` و `quickjs`) است و  به نصب جداگانه نیازی ندارد. داشتن حافظه خالی `حداقل 300 مگابایت` پیشنهاد می شود.
...
| `npm run electron:build` | ساخت نسخه پرتابل (`Jetro.exe`) + نصبی (`Jetro Setup.exe`) در پوشه `release/` |

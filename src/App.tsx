...
export default function App() {
  ...
  const addDl = async (url?: string, presetSavePath?: string, presetQueueId?: string) => {
    const u = (url || newUrl).trim();
    if (!u) return;
    if (!hasBackend()) { alert('Run via Electron (npm run app:dev) for real downloads.'); return; }
    setAdding(true);
    try {
      // savePath is now a folder — the filename is auto-detected from the URL.
      ...
      <div className="app-shell">
        ...
        <div className="main">
          <div className="topbar">
            <button className="btn btn-primary" onClick={() => { setSavePath(settings.downloadDir || ''); setNewQueueId(''); setShowAdd(true); }}><FiPlus className="btn-icon" /> New Download</button>
      ...
      {showAdd && (
        <div className="modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p>Choose a folder — Jetro saves the file inside with its original name.</p>
            <input className="input" autoFocus placeholder="https://example.com/file.zip" value={newUrl} onChange={(e) => setNewUrl(e.target.value)} />
            ...
            <div className="row">
              <select className="input" value={newConns} onChange={(e) => setNewConns(Number(e.target.value))}>
                {[1, 4, 8, 16, 32].map((n) => <option key={n} value={n}>{n} connections</option>)}
              </select>
              <select className="input" value={newQueueId} onChange={(e) => setNewQueueId(e.target.value)} title="Add to queue">
                <option value="">No queue</option>
                {queues.map((q) => (
            ...
            <div className="row" style={{ marginTop: 8 }}>
              <button className="btn" onClick={async () => { try { const t = await navigator.clipboard.readText(); if (t) setNewUrl(t.trim()); } catch {} }}><FiClipboard className="btn-icon" /> Paste from clipboard</button>
                ...
                <div className="complete-file-meta">
                  {pendingRemove.deleteFile ? (
                    ...
                    <>
                      ...
                      <span className="complete-dot-sep">•</span>
                      <span>{pendingRemoveItem.totalBytes ? `${((pendingRemoveItem.downloadedBytes / pendingRemoveItem.totalBytes) * 100).toFixed(1)}%` : fmtBytes(pendingRemoveItem.downloadedBytes)}</span>
                      <span className="complete-dot-sep">•</span>
